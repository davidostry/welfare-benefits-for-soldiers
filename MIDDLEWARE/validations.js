export async function validateBody(req, res, next) {
    ...
    if (benefitType === "giftCard" && !details.cardProvide || benefitType === "giftCard" && typeof details.cardProvide != "string") return res.status(400).json({ message: "valid cardProvide is missing" });
